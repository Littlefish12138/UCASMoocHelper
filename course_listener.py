...
2. 提供类 CourseHandler , 类中提供方法
    ...
    - run_video_task 根据要求完成所有视频的挂课/章节测试题
...
class CourseHandler:
    def __init__(self, page: ChromiumPage,
                  course_tree_config: dict,
                    *,
                    ...
                    answers: dict = {}):
        self.page = page
        #self.course_tree_config = deepcopy(course_tree_config)暂时没用
    ...
    def is_completed(self):
        """等待加载，查找任务点已完成元素是否存在，如果存在返回True/False，不存在则报错"""
    ...
    def finish_questions(self):
        """
        完成一个视频的章节测试题, 提取问题失败则中断并返回False, 答案异常则报错, 正常则返回True
        """
        ...
        try:
            ...
            self.log_callback(f"提取课程链接{self.page.url}的章节测试题时出错: {e}")
            return False
