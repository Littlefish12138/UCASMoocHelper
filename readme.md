...
本工具基于 **DrissionPage** 库（一个整合了 Selenium 和 Requests 优势的浏览器自动化框架），通过 **浏览器远程调试端口** 控制浏览器行为，无需繁琐的 WebDriver 配置。
...
- **`utils.py`** – 工具函数
  - 自动从注册表读取 Edge/Chrome 安装路径和用户数据目录

### 🔍 主要工作流程

1. 通过调试端口连接或启动浏览器，打开课程页面
2. 解析页面章节结构，生成待播放视频任务队列
3. 依次进入每个视频，检测是否已完成（任务点标记）
...
- 程序自动使用你的浏览器**用户数据目录**启动一个**带调试端口**的新浏览器实例
...
- 适用场景：你**自己**先用命令行启动带调试端口的浏览器，再让本工具连接上去
- 使用方法：
  1. 关闭所有浏览器窗口
  2. 用下面的命令启动浏览器（注意替换路径）
  ...
  **Chrome 示例：**
  ...
  3. 浏览器打开后，手动访问课程网站并登录
  4. 在工具界面选择“手动启动”，填入端口号（默认 9222），点击“开始挂课”即可

- ⚠️ 由于Google的安全策略，**使用 Chrome 时，绝对不能使用默认的用户数据目录**（如 `--user-data-dir="C:\Users\你的用户名\AppData\Local\Google\Chrome\User Data"`），否则程序无法连接。请务必指定一个**空白或新建的目录**，或者使用--incognito参数指定无痕模式。
...
**Q: 为什么连接失败**  
A: 通常是因为浏览器路径或用户数据目录设置错误。请检查配置，确保当前没有端口冲突，结束正在运行的浏览器实例。或者检查是否使用了Chrome的默认数据目录。
